...
All three prompt files have been upgraded from passive `mode: 'ask'` to active `mode: 'agent'` with tool capabilities.
...
NEW: "def test_calculate_total_with_valid_items_returns_correct_sum():
...
2. Finds existing test patterns in project
3. Identifies all dependencies to mock
4. Traces all code paths
5. Generates comprehensive test suite matching your style
```
